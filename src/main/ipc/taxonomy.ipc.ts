...
export function registerTaxonomyIpc(): void {
  ...
  ipcMain.handle(
    IPC.TAXONOMY_L2_CREATE,
    (_e, categoryL1Id: number, name: string) =>
      taxonomyRepo.createL2(categoryL1Id, name),
  );
  ipcMain.handle(
    IPC.TAXONOMY_L3_CREATE,
    (_e, categoryL2Id: number, name: string) =>
      taxonomyRepo.createL3(categoryL2Id, name),
  );
  ipcMain.handle(
    IPC.TAXONOMY_CATEGORY_UPDATE,
    (_e, level: "l1" | "l2" | "l3", id: number, name: string) => {
      taxonomyRepo.updateCategoryName(level, id, name);
    },
  );
  ipcMain.handle(IPC.TAXONOMY_FAV_ASSIGNMENTS, () =>
    taxonomyRepo.getFavoriteAssignments(),
  );
  ...
  ipcMain.handle(
    IPC.TAXONOMY_FAV_ORGANIZE_BILI,
    (_e, overrides?: FavFolderGroupOverrides) => {
  ...
  ipcMain.handle(IPC.TAXONOMY_FAV_MERGE_DUMP, () => {
    const taskId = favClassifyEngine.startMergeDumpIntoDefault();
    return { taskId };
  });
  ipcMain.handle(IPC.TAXONOMY_FAV_MERGE_DUP_TOPICS, () => {
    const taskId = favClassifyEngine.startMergeDuplicateTopics();
    return { taskId };
  });
  ipcMain.handle(IPC.TAXONOMY_FAV_TASK_STATUS, (_e, taskId: number) =>
    taxonomyRepo.getTask(taskId),
  );
  ...
  ipcMain.handle(IPC.TAXONOMY_UP_GROUP_TREE, () =>
    taxonomyRepo.getUpGroupTree(),
  );
  ipcMain.handle(
    IPC.TAXONOMY_UP_GROUP_CREATE,
    (_e, name: string, color?: string) =>
      taxonomyRepo.createUpGroup(name, color),
  );
  ipcMain.handle(
    IPC.TAXONOMY_UP_GROUP_MEMBERS,
    (_e, selection: UpGroupSelection) =>
