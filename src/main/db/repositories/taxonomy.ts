...
function migrateUpTaxonomyToV2(data: DbSchema): void {
  ...
  data.upMemberMap = [];
  data.nextIds.upGroup =
    Math.max(...data.upGroups.map((group) => group.id)) + 1;
...
export class TaxonomyRepository {
  ...
  createL2(categoryL1Id: number, name: string): CategoryL2 {
    const data = getDbData();
    const siblings = data.categoryL2.filter(
      (c) => c.categoryL1Id === categoryL1Id,
    );
  ...
  createL3(categoryL2Id: number, name: string): CategoryL3 {
    const data = getDbData();
    const siblings = data.categoryL3.filter(
      (c) => c.categoryL2Id === categoryL2Id,
    );
  ...
  findOrCreateL2(categoryL1Id: number, name: string): CategoryL2 {
    const data = getDbData();
    const existing = data.categoryL2.find(
      (item) => item.categoryL1Id === categoryL1Id && item.name === name,
    );
  ...
  findOrCreateL3(categoryL2Id: number, name: string): CategoryL3 {
    const data = getDbData();
    const existing = data.categoryL3.find(
      (item) => item.categoryL2Id === categoryL2Id && item.name === name,
    );
    if (existing) return existing;
  ...
  assignFavoritesBatch(
    ...
    data.favoriteItemMap = data.favoriteItemMap.filter(
      (item) => !replaceIds.has(item.mediaId),
    );
  ...
  createClassificationWriter() {
    ...
    const findL3InL1 = (
      ...
      for (const l3 of [...data.categoryL3, ...pendingL3]) {
        ...
        if (parent?.categoryL1Id === categoryL1Id && l3.name === name)
          return l3;
      }
      return undefined;
    };
    ...
    return {
      findL1ByName: (name: string) =>
        data.categoryL1.find((item) => item.name === name),
      ...
      findOrCreateL2: (categoryL1Id: number, name: string): CategoryL2 => {
        ...
        const siblings = [
          ...data.categoryL2.filter(
            (item) => item.categoryL1Id === categoryL1Id,
          ),
      ...
      findOrCreateL3: (categoryL2Id: number, name: string): CategoryL3 => {
        ...
        const siblings = [
          ...data.categoryL3.filter(
            (item) => item.categoryL2Id === categoryL2Id,
          ),
  ...
  repairTaxonomy(): void {
    ...
    const l1ByL2 = new Map(
      data.categoryL2.map((l2) => [l2.id, l2.categoryL1Id]),
    );
    ...
    for (const group of l3ByL1Name.values()) {
      ...
      const ranked = group
        .map((l3) => ({
          l3,
          count: data.favoriteItemMap.filter(
            (item) => item.categoryL3Id === l3.id,
          ).length,
        }))
    ...
    if (crossL3Remap.size > 0) {
      ...
      data.categoryL3 = data.categoryL3.filter(
        (l3) => !crossL3Remap.has(l3.id),
      );
    }
  ...
  matchesLocalCategory(
    mediaId: number,
    selection: {
      level: "all" | "l1" | "l2" | "l3" | "uncategorized";
      id: number | null;
    },
  ): boolean {
    ...
    if (selection.level === "uncategorized") {
      return (
        !assignment ||
        (!assignment.categoryL1Id &&
          !assignment.categoryL2Id &&
          !assignment.categoryL3Id)
      );
    }
    ...
    if (selection.level === "l2" && selection.id != null) {
      ...
      const l3Ids = data.categoryL3
        .filter((c) => c.categoryL2Id === selection.id)
        .map((c) => c.id);
      return (
        assignment.categoryL3Id != null &&
        l3Ids.includes(assignment.categoryL3Id)
      );
    }

    if (selection.level === "l1" && selection.id != null) {
      if (assignment.categoryL1Id === selection.id) return true;
      const l2Ids = data.categoryL2
        .filter((c) => c.categoryL1Id === selection.id)
        .map((c) => c.id);
      if (
        assignment.categoryL2Id != null &&
        l2Ids.includes(assignment.categoryL2Id)
      )
        return true;
      if (assignment.categoryL3Id != null) {
        const l3 = data.categoryL3.find(
          (c) => c.id === assignment.categoryL3Id,
        );
  ...
  getCategoryCounts(): Record<string, number> {
    ...
    for (const item of data.favoriteItemMap) {
      ...
      if (item.categoryL1Id)
        counts[`l1:${item.categoryL1Id}`] =
          (counts[`l1:${item.categoryL1Id}`] ?? 0) + 1;
      if (item.categoryL2Id)
        counts[`l2:${item.categoryL2Id}`] =
          (counts[`l2:${item.categoryL2Id}`] ?? 0) + 1;
      if (item.categoryL3Id)
        counts[`l3:${item.categoryL3Id}`] =
          (counts[`l3:${item.categoryL3Id}`] ?? 0) + 1;
    }
  ...
  getUpGroups(): UpGroup[] {
    ...
    return data.upGroups
      ...
      .map((group) => ({
        ...
        memberCount: data.upMemberMap.filter(
          (member) => member.upGroupId === group.id,
        ).length,
      }));
  }
  ...
  ensureExtendedUpTaxonomy(): void {
    ...
    for (const [index, seed] of UP_TAXONOMY_SEED.entries()) {
      let l1 = data.upGroups.find(
        (group) => group.parentId === null && group.name === seed.name,
      );
      if (!l1) {
        l1 = {
      ...
      .map((l1) => {
        const children = data.upGroups
          ...
          .map((l2) => ({
            ...
            color: l2.color,
            sortOrder: l2.sortOrder,
            count: data.upMemberMap.filter(
              (member) => member.upGroupId === l2.id,
            ).length,
          }));
  ...
  getUpGroupMemberMids(selection: UpGroupSelection): number[] {
    ...
    if (selection.level === "l2" && selection.id != null) {
      return members
        .filter((member) => member.upGroupId === selection.id)
    ...
    if (selection.level === "l1" && selection.id != null) {
      const childIds = new Set(
        data.upGroups
          .filter((group) => group.parentId === selection.id)
          .map((group) => group.id),
      );
      return members
        .filter((member) => childIds.has(member.upGroupId))
        .map((member) => member.mid);
    }
    ...
    return {
      findOrCreateL1(name: string): DbUpGroup {
        ...
        const item: DbUpGroup = {
          ...
          isAiGenerated: true,
          sortOrder: data.upGroups.filter((group) => group.parentId === null)
            .length,
          parentId: null,
        };
      ...
      findOrCreateL2(l1Id: number, name: string): DbUpGroup {
        const existing = data.upGroups.find(
          (group) => group.parentId === l1Id && group.name === name,
        );
        if (existing) return existing;
        ...
        const item: DbUpGroup = {
          ...
          isAiGenerated: true,
          sortOrder: data.upGroups.filter((group) => group.parentId === l1Id)
            .length,
          parentId: l1Id,
        };
        data.upGroups.push(item);
  ...
  assignUpsBatch(
    ...
    data.upMemberMap = data.upMemberMap.filter(
      (member) => !replaceMids.has(member.mid),
    );
