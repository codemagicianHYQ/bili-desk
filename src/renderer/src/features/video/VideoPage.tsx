          ...
          <div className="overflow-hidden rounded-2xl border border-border bg-card">
            {playInfo ? (
              ...
              <div className="relative aspect-video w-full bg-black">
                <BiliImage src={video.cover} alt={video.title} className="h-full w-full object-cover opacity-60" />
            ...
            <div className="space-y-4 p-6">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="min-w-0 flex-1">
                  <UpOwnerCard mid={video.owner.mid} name={video.owner.name} face={video.owner.face} />
                </div>
                <VideoFavButton aid={video.aid} />
              </div>

              <p className="text-xs text-muted-foreground">
                {formatCount(video.stat.view)} 播放 · {formatCount(video.stat.danmaku)} 弹幕
                {playInfo ? ` · ${playInfo.qualityLabel}` : ''}
                {video.stat.favorite > 0 ? ` · ${formatCount(video.stat.favorite)} 收藏` : ''}
              </p>
              <p className="text-sm leading-relaxed text-muted-foreground">{video.desc || '暂无简介'}</p>
              {playError && playInfo && <p className="text-sm text-red-400">{playError}</p>}
            </div>
          </div>
        </div>
      </div>
